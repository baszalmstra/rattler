...
fn probe_reflink_support(target_dir: &Path, package_dir: &Path, _fallback: bool) -> bool {
    ...
    // Probe by reflinking a small temporary file within the target directory.
    // Since the package directory resides on the same filesystem the result
    // also holds for cache-to-prefix reflinks.
    let uuid = uuid::Uuid::new_v4();
    let probe_src = target_dir.join(format!("reflink_probe_{uuid}"));
    let probe_dst = target_dir.join(format!("reflink_probe_dst_{uuid}"));
    let supported = std::fs::write(&probe_src, b"reflink probe").is_ok()
