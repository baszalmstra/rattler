    ...
    let tokens = match config.flow {
        ...
        OAuthFlow::Auto => {
            ...
            .await
            {
                ...
                Err(e) => {
                    eprintln!(
                        "Authorization code flow failed ({e}), trying device code flow..."
                    );
...
async fn discover_endpoints(
    ...
    let oidc_issuer = IssuerUrl::new(issuer_url.to_string())
        .map_err(|e| OAuthError::Discovery(e.to_string()))?;

    let provider_metadata =
        ExtendedCoreProviderMetadata::discover_async(oidc_issuer, http_client)
            .await
            .map_err(|e| OAuthError::Discovery(e.to_string()))?;
...
async fn device_code_flow(
    ...
    let device_auth: DeviceAuthResponse = resp
        .json()
        .await
        .map_err(|e| OAuthError::Authorization(format!("Failed to parse device auth response: {e}")))?;
    ...
    loop {
        ...
        let body: TokenResponse = resp
            .json()
            .await
            .map_err(|e| OAuthError::TokenExchange(format!("Failed to parse token response: {e}")))?;
