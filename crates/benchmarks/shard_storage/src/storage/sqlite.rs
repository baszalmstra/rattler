...
/// SQLite-based storage backend with optimized schema
...
impl SqliteStorage {
    ...
    /// Convert a Sha256Hash (hex string) to 32-byte array
...
impl ShardStorage for SqliteStorage {
    fn write_shard(&self, hash: &Sha256Hash, shard: &Shard) -> Result<()> {
        ...
        conn.execute(
            ...
            rusqlite::params![&hash_bytes[..], shard_bytes, created_at],
        )
...
mod tests {
    ...
    fn test_hash_conversion() {
        // Test hash to bytes and back
        let hash_str = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
        let hash = parse_digest_from_hex::<Sha256>(hash_str).unwrap();

        let bytes = SqliteStorage::hash_to_bytes(&hash).unwrap();
        assert_eq!(bytes.len(), 32);

        let hash2 = SqliteStorage::bytes_to_hash(&bytes).unwrap();
        assert_eq!(format!("{:x}", hash), format!("{:x}", hash2));
    }
}
