...
impl<'a, 'cache, D: DependencyProvider> Encoder<'a, 'cache, D> {
    ...
    fn on_requirement_candidates_available(
        ...
        // All matching candidates of a version set share that version set's
        // package name, so resolve the name once per candidate list instead of
        // once per candidate, and hoist the per-name bookkeeping of
        // `register_forbid_target` out of the inner loop as well.
    ...
    fn add_pending_forbid_clauses(&mut self) {
        for (name_id, candidate_vars) in self.pending_forbid_clauses.drain(..) {
            ...
            for candidate_var in candidate_vars {
                // Membership is globally deduplicated through `forbid_seen`,
                // so the variable is guaranteed to be new to the group.
